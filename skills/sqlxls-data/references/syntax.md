...
| `http` | `method`, `body`/`payload`, `headers`, `json_path`, `encoding`, `sheet` | `headers` 是 JSON 对象字符串；`${ENV}` 会展开。解码仍由 `format=` 决定。HTTPS 自签证书：`insecure=true` 或 `verify=false`（任意 https 定位符都可用，不必 format='http'） |
